...
No column uses the VARIANT type. DLT snapshot streams run APPLY CHANGES FROM
SNAPSHOT, which compares whole rows with the null-safe-equality operator
``<=>``; that operator cannot order ``VARIANT``, so any snapshot stream
carrying a Variant column fails at plan time
(``DATATYPE_MISMATCH.INVALID_ORDERING_TYPE``). Every field is therefore given a
concrete, ``<=>``-comparable type. Fields with a single documented shape are
modelled with concrete ``StructType`` / ``ArrayType`` / scalars (e.g. a
custom-field definition's ``company_ids`` is ``ARRAY<LONG>`` and ``options`` is
``ARRAY<STRING>``). The few genuinely free-form / polymorphic leaves —
``bills.tax.data`` (a union keyed by ``tax.regime``), the custom-field
``value`` payload, a custom-field definition's polymorphic ``default_value``,
and the recurring streams' untyped ``record_payload`` — are declared
``StringType`` and JSON-encoded in the read path (see
``_encode_free_form_leaves``). See ``dualentry_api_doc.md`` for extraction
notes.
